...
- **mustHaveCoverage**: For each must-have requirement provided, report whether it was visibly evidenced in the CV skim. Use the exact requirement wording. A requirement is only `present: true` if it registered without effort — buried evidence does not count.
...
- `mustHaveCoverage` is a screen-out test, not a comprehensiveness check. A recruiter screening on these requirements will reject without reading further if they don't see evidence immediately.
...
```json
{
  ...
  "gaps": [],
  "mustHaveCoverage": [
    { "requirement": "Experience managing complex end-to-end partnerships", "present": true },
    { "requirement": "C-level executive relationship experience", "present": false }
  ]
}
```
